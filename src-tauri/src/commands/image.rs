...
fn prepare_node_image_from_bytes(
    ...
    let placeholder_image_path = if bypass_preview {
        image::load_from_memory(bytes).ok().and_then(|tiny| {
            let (tw, th) = (32u32, 32u32);
            let resized = tiny.resize(tw, th, image::imageops::FilterType::Triangle);
            let mut buf = Cursor::new(Vec::new());
            resized.write_to(&mut buf, image::ImageFormat::Png).ok()?;
            persist_image_bytes(app, buf.get_ref(), "png").ok()
        })
    } else {
        None
    };
    ...
    let placeholder_image_path = {
        let tiny = resized.resize(32, 32, image::imageops::FilterType::Triangle);
        let mut buf = Cursor::new(Vec::new());
