...
use crate::ai::{
    ...
    ProviderTaskSubmission, VideoGenerateRequest,
};
...
pub async fn submit_generate_video_job(
    ...
    let spawned_provider = provider.clone();
    tauri::async_runtime::spawn(async move {
        let result = spawned_provider.generate_video(req).await;
        let update_result = match result {
            Ok(video_source) => update_generation_job(
                &app_handle,
                spawned_job_id.as_str(),
                "succeeded",
                Some(video_source.as_str()),
                None,
            ),
            Err(error) => {
                let message = error.to_string();
        ...
        if let Err(error) = update_result {
            info!("Failed to update video generation job: {}", error);
        }
        let mut active_set = active_non_resumable_job_ids().write().await;
        active_set.remove(spawned_job_id.as_str());
    });
