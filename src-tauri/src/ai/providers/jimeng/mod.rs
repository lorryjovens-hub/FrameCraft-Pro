...
impl AIProvider for JimengProvider {
    fn name(&self) -> &str {
        "jimeng"
    }
    ...
    async fn generate(&self, _request: GenerateRequest) -> Result<String, AIError> {
        Err(AIError::Provider("Jimeng provider does not support image generation, only video".to_string()))
    }
    ...
    async fn generate_video(&self, request: VideoGenerateRequest) -> Result<String, AIError> {
        ...
        info!(
            "[Jimeng Request] model: {}, duration: {}, aspect_ratio: {}",
            request.model, request.duration, aspect_ratio
        );

        let submit_endpoint = format!("{}/api/v1/video/generate", JIMENG_BASE_URL);
        let mut input = json!({
            "model": model_path,
        ...
        if !submit_response.status().is_success() {
            ...
            return Err(AIError::Provider(format!(
                "Jimeng submit failed {}: {}",
                status, error_text
            )));
        }

        let submit_raw = submit_response.text().await.unwrap_or_default();
        let submit_body = serde_json::from_str::<JimengSubmitResponse>(&submit_raw).map_err(|err| {
            AIError::Provider(format!(
                "Jimeng submit invalid JSON response: {}; raw={}",
                err, submit_raw
            ))
        })?;
        ...
        let task_id = submit_body.task_id.or_else(|| {
            ...
            AIError::Provider(format!("Jimeng submit returned no task_id: {}", submit_raw))
        })?;

        loop {
            sleep(Duration::from_millis(POLL_INTERVAL_MS)).await;

            let status_endpoint = format!("{}/api/v1/video/status/{}", JIMENG_BASE_URL, task_id);
            ...
            if !status_response.status().is_success() {
                ...
                return Err(AIError::Provider(format!(
                    "Jimeng status check failed {}: {}",
                    status, error_text
                )));
            }

            let status_raw = status_response.text().await.unwrap_or_default();
            let status_body = serde_json::from_str::<JimengStatusResponse>(&status_raw).map_err(|err| {
                AIError::Provider(format!(
                    "Jimeng status invalid JSON response: {}; raw={}",
                    err, status_raw
                ))
            })?;
            ...
            match status_body.status.as_str() {
                "pending" | "processing" | "running" => {
                    info!("[Jimeng] Task {} status: {}", task_id, status_body.status);
                    continue;
                }
                "completed" | "succeeded" => {
                    ...
                    return Err(AIError::Provider("Jimeng completed but no video_url returned".to_string()));
                }
                "failed" | "error" => {
                    return Err(AIError::TaskFailed(status_body.error_message.unwrap_or_else(|| "Unknown error".to_string())));
                }
                _ => {
                    return Err(AIError::Provider(format!("Jimeng unexpected status: {}", status_body.status)));
                }
            }
        }
    }
