...
interface CanvasState {
  ...
  addNodes: (newNodes: CanvasNode[]) => void;
  addEdges: (newEdges: CanvasEdge[]) => void;
...
function normalizeHistory(history?: CanvasHistoryState): CanvasHistoryState {
  ...
  const normalizeEntry = (entry: CanvasHistoryEntry): CanvasHistoryEntry => {
    const normalizedNodes = normalizeNodes(entry.snapshot.nodes);
    return {
      ...entry,
      snapshot: {
        nodes: normalizedNodes,
        edges: normalizeEdgesWithNodes(entry.snapshot.edges, normalizedNodes),
      },
    };
  };

  return {
    past: history.past.slice(-MAX_HISTORY_STEPS).map(normalizeEntry),
    future: history.future.slice(-MAX_HISTORY_STEPS).map(normalizeEntry),
  };
}
...
function pushHistoryEntry(
  ...
  if (last && last.snapshot.nodes === entry.snapshot.nodes && last.snapshot.edges === entry.snapshot.edges) {
    return entries;
  }
...
function pushHistory(
  ...
  const entry = createHistoryEntry(nodes, edges, operationType, description, affectedNodeIds);
  return {
    past: pushHistoryEntry(historyState.past, entry),
    future: [],
  };
}
...
export const useCanvasStore = create<CanvasState>((set, get) => ({
  ...
  onNodesChange: (changes) => {
    set((state) => {
      ...
      if (hasInteractionEnd) {
        ...
        const opType: HistoryOperationType = hasResizeEnd ? 'node_resize' : 'node_move';
        nextHistory = pushHistory(state.history, snapshot.nodes, snapshot.edges, opType, opType === 'node_resize' ? '调整节点尺寸' : '移动节点', affectedIds);
        nextDragHistorySnapshot = null;
      } else if (hasMeaningfulChange && !hasInteractionMove) {
        nextHistory = pushHistory(state.history, state.nodes, state.edges, 'node_data_change', '节点数据变更', []);
        nextDragHistorySnapshot = null;
      }
  ...
  onEdgesChange: (changes) => {
    set((state) => {
      ...
      return {
        edges: nextEdges,
        history: pushHistory(state.history, state.nodes, nextEdges, 'node_disconnect', '删除连接'),
        dragHistorySnapshot: null,
      };
    });
  },
  ...
  onConnect: (connection) => {
    ...
    set((state) => ({
      ...
      history: pushHistory(state.history, state.nodes, state.edges, 'node_connect', '创建连接'),
      dragHistorySnapshot: null,
    }));
  },
  ...
  addNode: (type, position, data = {}) => {
    ...
    const nodeTypeLabels: Record<CanvasNodeType, string> = {
      uploadNode: '上传节点',
      imageNode: '编辑节点',
      exportImageNode: '导出节点',
      exportVideoNode: '导出视频节点',
      storyboardNode: '故事板节点',
      storyboardGenNode: 'AI生成节点',
      groupNode: '分组节点',
      textAnnotationNode: '文本标注节点',
      videoGenNode: '视频生成节点',
    };
    set({
      nodes: [...state.nodes, newNode],
      history: pushHistory(state.history, state.nodes, state.edges, 'node_add', `添加${nodeTypeLabels[type] || '节点'}`),
      dragHistorySnapshot: null,
    });
    return newNode.id;
  },
  ...
  addNodes: (newNodes: CanvasNode[]) => {
    const state = get();
    set({
      nodes: [...state.nodes, ...newNodes],
      history: pushHistory(state.history, state.nodes, state.edges, 'node_add', `添加${newNodes.length}个节点`),
      dragHistorySnapshot: null,
    });
  },

  addEdges: (newEdges: CanvasEdge[]) => {
    const state = get();
    set({
  ...
  addDerivedUploadNode: (sourceNodeId, imageUrl, aspectRatio, previewImageUrl) => {
    ...
    set({
      ...
      activeToolDialog: null,
      history: pushHistory(state.history, state.nodes, state.edges, 'node_add', '添加派生上传节点'),
      dragHistorySnapshot: null,
    });
  ...
  addDerivedExportNode: (sourceNodeId, imageUrl, aspectRatio, previewImageUrl, options) => {
    ...
    set({
      ...
      selectedNodeId: node.id,
      activeToolDialog: null,
      history: pushHistory(state.history, state.nodes, state.edges, 'node_add', '添加导出节点'),
      dragHistorySnapshot: null,
    });
  ...
  addStoryboardSplitNode: (sourceNodeId, rows, cols, frames, frameAspectRatio) => {
    ...
    set({
      ...
      selectedNodeId: node.id,
      activeToolDialog: null,
      history: pushHistory(state.history, state.nodes, state.edges, 'node_add', '添加故事板节点'),
      dragHistorySnapshot: null,
    });
  ...
  updateNodeData: (nodeId, data) => {
    set((state) => {
      ...
      return {
        nodes: nextNodes,
        history: pushHistory(state.history, nextNodes, state.edges, 'node_data_change', '更新节点数据', [nodeId]),
        dragHistorySnapshot: null,
      };
    });
  },
  ...
  updateStoryboardFrame: (nodeId, frameId, data) => {
    set((state) => {
      ...
      return {
        nodes: nextNodes,
        history: pushHistory(state.history, nextNodes, state.edges, 'node_move', '更新节点位置', [nodeId]),
        dragHistorySnapshot: null,
      };
    });
  },
  ...
  reorderStoryboardFrame: (nodeId, draggedFrameId, targetFrameId) => {
    set((state) => {
      ...
      return {
        nodes: nextNodes,
        history: pushHistory(state.history, nextNodes, state.edges, 'frame_reorder', '重排分镜帧', [nodeId]),
        dragHistorySnapshot: null,
      };
    });
  },
  ...
  deleteNodes: (nodeIds) => {
    ...
    set((state) => {
      ...
      return {
        ...
        history: pushHistory(state.history, nextNodes, nextEdges, 'node_delete', `删除${deleteSet.size}个节点`, Array.from(deleteSet)),
        dragHistorySnapshot: null,
      };
    });
  },
    ...
    set({
      ...
      activeToolDialog:
        state.activeToolDialog && memberSet.has(state.activeToolDialog.nodeId)
          ? null
          : state.activeToolDialog,
      history: pushHistory(state.history, [...state.nodes, groupNode], state.edges, 'group_create', '创建节点分组', [groupNode.id]),
      dragHistorySnapshot: null,
    });
  ...
  ungroupNode: (groupNodeId) => {
    ...
    set({
      ...
      history: pushHistory(state.history, nextNodes, nextEdges, 'group_delete', '取消节点分组', [groupNodeId]),
      dragHistorySnapshot: null,
    });
  ...
  deleteEdge: (edgeId) => {
    set((state) => {
      ...
      return {
        ...
        history: pushHistory(state.history, state.nodes, state.edges, 'node_disconnect', '删除连接'),
        dragHistorySnapshot: null,
      };
    });
  },
  ...
  undo: () => {
    ...
    const currentEntry = createHistoryEntry(currentSnapshot.nodes, currentSnapshot.edges, 'unknown', '撤销恢复');

    set({
      nodes: target.snapshot.nodes,
      edges: target.snapshot.edges,
      selectedNodeId: resolveSelectedNodeId(state.selectedNodeId, target.snapshot.nodes),
      activeToolDialog: resolveActiveToolDialog(state.activeToolDialog, target.snapshot.nodes),
      history: {
        past: nextPast,
        future: pushHistoryEntry(state.history.future, currentEntry),
      },
      dragHistorySnapshot: null,
    });
    return true;
  ...
  redo: () => {
    ...
    const currentSnapshot = createSnapshot(state.nodes, state.edges);
    const currentEntry = createHistoryEntry(currentSnapshot.nodes, currentSnapshot.edges, 'unknown', '重做恢复');
    ...
    set({
      ...
      activeToolDialog: resolveActiveToolDialog(state.activeToolDialog, target.snapshot.nodes),
      history: {
        past: pushHistoryEntry(state.history.past, currentEntry),
        future: nextFuture,
      },
      dragHistorySnapshot: null,
    });
  ...
  clearCanvas: () => {
    set((state) => {
      ...
      return {
        ...
        selectedNodeId: null,
        activeToolDialog: null,
        history: pushHistory(state.history, [], [], 'batch_operation', '清空画布'),
        dragHistorySnapshot: null,
      };
    });
  },
