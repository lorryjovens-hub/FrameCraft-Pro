...
import { NodeGroupTemplateDialog } from './ui/NodeGroupTemplateDialog';
...
const HANDLE_ATTRACT_DISTANCE_PX = 30;
...
function findNearestHandle(
...
): { nodeId: string; handleType: 'source' | 'target'; x: number; y: number } | null {
  let nearestHandle: { nodeId: string; handleType: 'source' | 'target'; x: number; y: number; distance: number } | null = null;
  ...
  for (const node of nodes) {
    ...
    const handleEl = nodeElement.querySelector<HTMLElement>(`.react-flow__handle-${handleType}`);
    ...
    const dx = clientX - handleCenterX;
    const dy = clientY - handleCenterY;
    const distance = Math.sqrt(dx * dx + dy * dy);

    if (distance <= HANDLE_ATTRACT_DISTANCE_PX && (!nearestHandle || distance < nearestHandle.distance)) {
      nearestHandle = {
        nodeId: node.id,
        handleType,
  ...
  return nearestHandle ? { nodeId: nearestHandle.nodeId, handleType: nearestHandle.handleType, x: nearestHandle.x, y: nearestHandle.y } : null;
}
...
export function Canvas() {
        ...
        try {
          while (true) {
            ...
            const status = await canvasAiGateway.getGenerateVideoJob(jobId).catch((error) => {
              console.warn('[VideoGenerationJob] poll failed', { nodeId: pendingNode.id, jobId, error });
              return null;
            });
            if (!status) {
              await sleep(GENERATION_JOB_POLL_INTERVAL_MS);
  ...
  const handleConnectEnd = useCallback(
    (event: MouseEvent | TouchEvent, connectionState: FinalConnectionState) => {
      ...
      const oppositeHandleType: 'source' | 'target' = pendingConnectStart.handleType === 'source' ? 'target' : 'source';
      const nearestHandle = findNearestHandle(
        clientPosition.x,
        clientPosition.y,
        nodes,
      ...
      let attractedEndX = endX;
      let attractedEndY = endY;
      if (nearestHandle) {
        attractedEndX = nearestHandle.x;
        attractedEndY = nearestHandle.y;
      }
      ...
      if (startX === null || startY === null) {
        ...
        setPreviewConnectionVisual({
          d: createPreviewPath({
            ...
            end: { x: attractedEndX, y: attractedEndY },
            handleType: pendingConnectStart.handleType,
          }),
          stroke: nearestHandle ? 'rgb(var(--accent-rgb))' : 'rgba(255,255,255,0.9)',
          strokeWidth: nearestHandle ? 2.5 : 1,
          strokeLinecap: 'round',
          left: 0,
          top: 0,
      ...
      if (nearestHandle) {
        const sourceNode =
          pendingConnectStart.handleType === 'source'
            ? nodes.find((node) => node.id === pendingConnectStart.nodeId)
            : nodes.find((node) => node.id === nearestHandle.nodeId);
        const targetNode =
          pendingConnectStart.handleType === 'source'
            ? nodes.find((node) => node.id === nearestHandle.nodeId)
            : nodes.find((node) => node.id === pendingConnectStart.nodeId);
  ...
  return (
    <div ref={wrapperRef} className="relative h-full w-full">
      ...
      {nodes.length > 0 && (
        <button
          type="button"
          className="absolute bottom-4 right-4 z-30 flex h-10 items-center gap-2 rounded-full border border-[rgba(255,255,255,0.18)] bg-bg-dark/70 px-4 text-sm text-text-dark backdrop-blur-sm transition-colors hover:bg-bg-dark"
