...
import { InpaintToolEditor } from './tool-editors/InpaintToolEditor';
import { BrushMaskEditor } from './tool-editors/BrushMaskEditor';
...
export function NodeToolDialog() {
  ...
  const resolveResultNodeTitle = useCallback((toolType: NodeToolType | undefined) => {
    ...
    if (toolType === NODE_TOOL_TYPES.inpaint) {
      return t('toolDialog.inpaintResultTitle');
    }
  ...
  const widthClassName = useMemo(() => {
    ...
    if (activePlugin.editor === 'inpaint') {
      return 'w-[min(980px,calc(100vw-40px))]';
    }
    if (activePlugin.editor === 'brush') {
      return 'w-[min(1100px,calc(100vw-40px))]';
    }
    return 'w-[min(460px,calc(100vw-40px))]';
  }, [activePlugin]);
  ...
  const editorContent = useMemo(() => {
    ...
    if (activePlugin.editor === 'inpaint' && sourceImageUrl) {
      return (
        <InpaintToolEditor
          plugin={activePlugin}
          sourceImageUrl={sourceImageUrl}
    ...
    if (activePlugin.editor === 'brush' && sourceImageUrl) {
      return (
        <BrushMaskEditor
          sourceImageUrl={sourceImageUrl}
          options={options}
