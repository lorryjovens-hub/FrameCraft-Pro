...
import { Copy, Crop, Download, FolderOpen, LayoutGrid, Paintbrush, PenLine, RefreshCw, Scissors, Trash2, Unlink2 } from 'lucide-react';
...
import {
  MOBILE_TOOLBAR_BUTTON_CLASS,
  MOBILE_TOOLBAR_ICON_CLASS,
} from './nodeControlStyles';

interface NodeActionToolbarProps {
  node: CanvasNode;
  onOpenTemplateSave?: (groupNodeId: string) => void;
}
...
export const NodeActionToolbar = memo(({ node, onOpenTemplateSave }: NodeActionToolbarProps) => {
  ...
  const handleMobileToolAction = useCallback((action: () => void) => {
    if (isMobileDevice) {
      action();
  ...
  const resolveToolLabel = useCallback((toolType: NodeToolType) => {
    ...
    if (toolType === NODE_TOOL_TYPES.inpaint) {
      return t('tool.inpaint');
    }
    return '';
  }, [t]);
  ...
  return (
    <>
      ...
      <UiPanel className="flex items-center gap-1 rounded-full p-1">
        ...
        {!isImageEdit && isGroupNode(node) && onOpenTemplateSave && (
          <UiChipButton
            key="group-template"
            className={`h-8 ${TOOLBAR_BUTTON_RADIUS_CLASS} px-2.5 text-xs ${TOOLBAR_NEUTRAL_BUTTON_CLASS}`}
            onClick={(event) => {
              event.stopPropagation();
              onOpenTemplateSave(node.id);
            }}
          >
    ...
    {isMobileDevice && (
        <MobileBottomDrawer
          ...
          title={t('nodeToolbar.tools')}
        >
          <div className="flex flex-wrap gap-2">
            {canHandleImage && (
              <UiChipButton
                key="image-copy"
                className={`${MOBILE_TOOLBAR_BUTTON_CLASS} ${TOOLBAR_NEUTRAL_BUTTON_CLASS}`}
                onClick={() => {
              ...
              >
                <Copy className={MOBILE_TOOLBAR_ICON_CLASS} />
                {t('nodeToolbar.copy')}
              </UiChipButton>
            )}
            {!isImageEdit && canHandleImage && (
              ...
              >
                <Download className={MOBILE_TOOLBAR_ICON_CLASS} />
                {t('nodeToolbar.download')}
              </UiChipButton>
            )}
            {tools.map((tool) => {
              ...
              return (
                ...
                >
                  <Icon className={MOBILE_TOOLBAR_ICON_CLASS} />
                  {resolveToolLabel(tool.type)}
                </UiChipButton>
              );
            })}
            {canCopyStoryboardText && (
              ...
              >
                <Copy className={MOBILE_TOOLBAR_ICON_CLASS} />
                {isCopyTextSuccess ? t('common.success') : t('nodeToolbar.copyText')}
              </UiChipButton>
            )}
            {!isImageEdit && canReupload && (
              <UiChipButton
                key="image-regenerate"
                className={`${MOBILE_TOOLBAR_BUTTON_CLASS} ${TOOLBAR_NEUTRAL_BUTTON_CLASS}`}
                onClick={() => {
                  const nodeId = node.id;
                  handleMobileToolAction(() => {
                    canvasEventBus.publish('upload-node/reupload', { nodeId });
                  });
                }}
              >
                <RefreshCw className={MOBILE_TOOLBAR_ICON_CLASS} />
                {t('nodeToolbar.regenerate')}
              </UiChipButton>
            )}
            {!isImageEdit && isGroupNode(node) && onOpenTemplateSave && (
              <UiChipButton
                ...
                onClick={() => {
                  const nodeId = node.id;
                  handleMobileToolAction(() => {
                    onOpenTemplateSave(nodeId);
                  });
                }}
              >
                <LayoutGrid className={MOBILE_TOOLBAR_ICON_CLASS} />
                {t('template.saveAsTemplate')}
              </UiChipButton>
            )}
            {!isImageEdit && isGroupNode(node) && (
              <UiChipButton
                key="group-ungroup"
                className={`${MOBILE_TOOLBAR_BUTTON_CLASS} border-amber-500/45 bg-amber-500/15 text-red-300 hover:bg-amber-500/25`}
                onClick={() => {
                  const nodeId = node.id;
                  handleMobileToolAction(() => {
                    ungroupNode(nodeId);
              ...
              >
                <Unlink2 className={MOBILE_TOOLBAR_ICON_CLASS} />
                {t('nodeToolbar.ungroup')}
              </UiChipButton>
            )}
            <UiChipButton
              key="node-delete"
              className={`${MOBILE_TOOLBAR_BUTTON_CLASS} border-red-500/45 bg-red-500/15 text-red-300 hover:bg-red-500/25`}
              onClick={() => {
                const nodeId = node.id;
                handleMobileToolAction(() => {
                  deleteNode(nodeId);
            ...
            >
              <Trash2 className={MOBILE_TOOLBAR_ICON_CLASS} />
              {t('common.delete')}
            </UiChipButton>
          </div>
        </MobileBottomDrawer>
