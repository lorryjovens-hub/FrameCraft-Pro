...
interface ModelParamsControlsProps {
  ...
  selectedModel: ImageModelDefinition | VideoModelDefinition;
...
export const ModelParamsControls = memo(({
  ...
  const providerOptions = useMemo(() => {
    const providerOrder = ['kie', 'ppio', 'fal', 'grsai', 'jimeng', 'wanxiang'];
    const providerIndex = new Map(providerOrder.map((id, index) => [id, index]));
    const allModels = [...imageModels, ...videoModels];
    const uniqueProviderIds = Array.from(new Set(allModels.map((model) => model.providerId)));
  ...
  const providerVideoModels = useMemo(
  ...
  const videoModelGroups = useMemo(() => {
    if (providerVideoModels.length === 0) return [];
    const grouped = new Map<string, VideoModelDefinition[]>();
    providerVideoModels.forEach((model) => {
      const normalizedName = model.displayName.replace(/\s*\([^)]*\)\s*$/u, '').trim();
      const key = normalizedName.length > 0 ? normalizedName : model.displayName;
      const current = grouped.get(key) ?? [];
      current.push(model);
  ...
  const isMediaTypeVideo = selectedModel.mediaType === 'video';
        ...
        >
          <UiPanel className={modelPanelClassName}>
            <div className="ui-scrollbar max-h-[340px] space-y-4 overflow-y-auto p-1">
              ...
              <section>
                <div className="mb-2 text-xs font-medium text-text-muted">
                  {isMediaTypeVideo ? '视频模型' : t('modelParams.model')}
                </div>
                <div className="flex flex-wrap gap-2">
                  {isMediaTypeVideo ? (
                    videoModelGroups.length > 0 ? (
                      ...
                      <div className="text-xs text-text-muted">暂无可用视频模型</div>
                    )
                  ) : (
          ...
          <UiPanel className={paramsPanelClassName}>
              ...
              <div className="grid grid-cols-5 gap-1 rounded-xl border border-[rgba(255,255,255,0.1)] bg-bg-dark/65 p-1">
                {aspectRatioOptions.map((item) => {
                  ...
                  return (
                    <button
                      key={item.value}
                      className={`rounded-lg px-0.5 py-1.5 transition-colors ${active
                        ? 'bg-surface-dark text-text-dark'
                        : 'text-text-muted hover:bg-bg-dark'
                        }`}
                      onClick={(event) => {
                    ...
                    >
                      ...
                      <div className="truncate text-[10px] leading-tight">{item.label}</div>
                    </button>
                  );
                })}
              </div>
            ...
            {panelExtraParamSchema.length > 0 && (
              <div className="mt-3">
                ...
                <div className="space-y-2 rounded-xl border border-[rgba(255,255,255,0.1)] bg-bg-dark/65 p-2">
                  {panelExtraParamSchema.map((definition) => {
                    ...
                    return (
                      <div key={definition.key} className="space-y-1.5 rounded-lg border border-[rgba(255,255,255,0.08)] bg-bg-dark/40 px-2.5 py-2">
                        <div>
                        ...
                        {definition.type === 'boolean' && (
                          <label className="flex cursor-pointer items-center gap-2 text-xs text-text-dark">
                            <UiCheckbox
            ...
            <div className="space-y-3">
              ...
              {inlineExtraParamSchema.map((definition) => {
                ...
                return (
                  <div
                    key={definition.key}
                    className="space-y-1.5 rounded-lg border border-[rgba(255,255,255,0.08)] bg-bg-dark/40 px-2.5 py-2"
                  >
                    <div>
                      <div className="text-[11px] font-medium text-text-dark">{translatedLabel}</div>
                      {translatedDescription && (
