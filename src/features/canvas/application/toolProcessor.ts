...
export class CanvasToolProcessor implements ToolProcessor {
  ...
  async process(
    ...
    switch (toolType) {
      ...
      case NODE_TOOL_TYPES.inpaint:
        return {
          outputImageUrl: await this.inpaintImage(sourceImageUrl, options),
        };
      case NODE_TOOL_TYPES.brushInpaint:
        return {
          outputImageUrl: await this.brushInpaintImage(sourceImageUrl, options),
        };
      case NODE_TOOL_TYPES.multiAngleRender:
        return {
          outputImageUrl: await this.multiAngleRenderImage(sourceImageUrl, options),
        };
      case NODE_TOOL_TYPES.productPhotography:
        return {
          outputImageUrl: await this.productPhotographyImage(sourceImageUrl, options),
        };
      case NODE_TOOL_TYPES.lightingAdjustment:
        return {
          outputImageUrl: await this.lightingAdjustmentImage(sourceImageUrl, options),
        };
      default:
  ...
  private async inpaintImage(sourceImageUrl: string, options: Record<string, unknown>): Promise<string> {
    const prompt = String(options.prompt ?? '').trim();
    if (!prompt) {
      throw new Error('请输入提示词');
    }

    const sourceImage = await loadImageElement(sourceImageUrl);
    ...
    if (hasMaskArea) {
      ...
      if (maskContext) {
        ...
        maskContext.fillRect(
          Math.max(0, Math.min(sourceImage.naturalWidth - 1, Math.floor(cropX))),
          Math.max(0, Math.min(sourceImage.naturalHeight - 1, Math.floor(cropY))),
          Math.max(1, Math.min(Math.floor(cropWidth), sourceImage.naturalWidth - Math.floor(cropX))),
          Math.max(1, Math.min(Math.floor(cropHeight), sourceImage.naturalHeight - Math.floor(cropY)))
        );
        maskBase64 = canvasToDataUrl(maskCanvas);
      }
    }

    const { canvasAiGateway } = await import('./canvasServices');

    const sourceDataUrl = canvasToDataUrl(canvas);

    const result = await canvasAiGateway.generateImage({
      prompt,
      model: 'kie/nano-banana-2',
      size: '1K',
      aspectRatio: String(options.aspectRatio ?? '1:1'),
      referenceImages: [sourceDataUrl],
      extraParams: maskBase64 ? { mask_image: maskBase64 } : undefined,
    });

    return result;
  }

  private async brushInpaintImage(sourceImageUrl: string, options: Record<string, unknown>): Promise<string> {
    const prompt = String(options.prompt ?? '').trim();
    if (!prompt) {
      throw new Error('请输入提示词');
    }

    const maskImage = options.maskImage as string | null;
    if (!maskImage) {
      throw new Error('请先在图像上绘制要重绘的区域');
    }
    ...
    maskContext.drawImage(maskImg, 0, 0, sourceImage.naturalWidth, sourceImage.naturalHeight);

    const { canvasAiGateway } = await import('./canvasServices');

    const sourceDataUrl = canvasToDataUrl(canvas);
    const maskBase64 = canvasToDataUrl(maskCanvas);
    ...
    const result = await canvasAiGateway.generateImage({
      ...
      aspectRatio: String(options.aspectRatio ?? '1:1'),
      referenceImages: [sourceDataUrl],
      extraParams: { mask_image: maskBase64 },
    });
  ...
  private async multiAngleRenderImage(sourceImageUrl: string, options: Record<string, unknown>): Promise<string> {
    const prompt = String(options.prompt ?? '').trim();
    if (!prompt) {
      throw new Error('请输入提示词');
    }
  ...
  private async productPhotographyImage(sourceImageUrl: string, options: Record<string, unknown>): Promise<string> {
    const prompt = String(options.prompt ?? '').trim();
    if (!prompt) {
      throw new Error('请输入提示词');
    }

    const style = String(options.style ?? 'professional');
    ...
    const result = await canvasAiGateway.generateImage({
      prompt: `${prompt}，${stylePrompt}，专业产品摄影风格`,
      model: 'kie/nano-banana-2',
      size: '1K',
      aspectRatio: String(options.aspectRatio ?? '1:1'),
      referenceImages: [sourceDataUrl],
  ...
  private async lightingAdjustmentImage(sourceImageUrl: string, options: Record<string, unknown>): Promise<string> {
    const lightAngle = Number(options.lightAngle ?? 45);
    const lightColor = String(options.lightColor ?? '#ffffff');
    const lightIntensity = Number(options.lightIntensity ?? 100);
    ...
    const result = await canvasAiGateway.generateImage({
      prompt: `调整打光效果，光源角度${lightAngle}度，光源颜色${lightColor}，光照强度${lightIntensity}%，保持产品细节清晰`,
      model: 'kie/nano-banana-2',
      size: '1K',
      aspectRatio: String(options.aspectRatio ?? '1:1'),
      referenceImages: [sourceDataUrl],
  ...
  private getStylePrompt(style: string): string {
    switch (style) {
      case 'professional':
        return '高级质感，专业灯光，清晰细节';
      case 'fashion':
        return '时尚潮流，现代感，创意构图';
      case 'minimalist':
        return '简约清新，干净背景，柔和光线';
      case 'tech':
        return '科技感，未来主义，蓝色调灯光';
      default:
        return '专业质感，清晰细节';
    }
  }
